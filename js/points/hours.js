...
function load() {
    document.getElementById('embed').value = "https://mgcounts.com/embeds/hours?id=" + id + ""
    ...
    fetch('https://backend.mgcounts.com/streamlabs/search/' + id + '')
        ...
        .then(data => {
            ...
            if (data.cloudbot.data.length > 0) {
                document.getElementById('count').innerHTML = data.cloudbot.data[0].time
                lastest = parseFloat(data.cloudbot.data[0].time)
                ...
                chart.series[0].addPoint([Date.now(), Math.floor(data.cloudbot.data[0].time)])
                if (banner == null) {
                    document.getElementById('banner').src = image
                } else {
                    document.getElementById('banner').src = banner
                }
                if (data.cloudbot.data[0].hours < 10) return 10; let x1 = Math.floor(Math.log10(data.cloudbot.data[0].time)); let x2 = Math.ceil(data.cloudbot.data[0].time / 10 ** x1); let x3 = x2 * 10 ** x1; let goal = x3 - data.cloudbot.data[0].time;
                document.getElementById('count_2').innerHTML = goal
                document.getElementById('name_2').innerHTML = "Goal"
                document.getElementById('count_1').innerHTML = data.cloudbot.data[0].points
                document.getElementById('name_1').innerHTML = 'Points'
            } else {
