...
let id = "straight from mg";
let last = []
if (url.searchParams.get('id')) {
    id = params.get("id");
}
function load() {
    document.getElementById('embed').value = "https://mgcounts.com/embeds/points?id=" + id + ""
...
let banner = null;
function fetcher() {
    fetch('https://backend.mgcounts.com/streamlabs/search/' + id + '')
        .then(response => response.json())
        .then(data => {
            if (image == false) {
                fetch('https://backend.mgcounts.com/youtube/user/' + data.cloudbot.data[0].viewer.platform_id + '')
                    .then(response => response.json())
                    .then(data2 => {
                        image = data2.image
                        if (data2.banner) {
                            banner = data2.banner.bannerExternalUrl
                            document.getElementById('img').src = image
                            document.getElementById('banner').src = banner
                        }
                    })
            }
            if (data.cloudbot.data.length > 0) {
                document.getElementById('count').innerHTML = data.cloudbot.data[0].points
                lastest = parseFloat(data.cloudbot.data[0].points)
                document.getElementById('name').innerHTML = data.cloudbot.data[0].viewer.name
                document.getElementById('img').src = image
                if (chart.series[0].points.length == config.graphLength) chart.series[0].data[0].remove();
                chart.series[0].addPoint([Date.now(), Math.floor(data.cloudbot.data[0].points)])
                if (banner == null) {
                    document.getElementById('banner').src = image
                } else {
                    document.getElementById('banner').src = banner
                }
                if (data.cloudbot.data[0].points < 10) return 10; let x1 = Math.floor(Math.log10(data.cloudbot.data[0].points)); let x2 = Math.ceil(data.cloudbot.data[0].points / 10 ** x1); let x3 = x2 * 10 ** x1; let goal = x3 - data.cloudbot.data[0].points;
                document.getElementById('count_2').innerHTML = goal
                document.getElementById('name_2').innerHTML = "Goal"
                document.getElementById('count_1').innerHTML = data.cloudbot.data[0].time
                document.getElementById('name_1').innerHTML = 'Hours'
            } else {
...
function search() {
    fetch('https://backend.mgcounts.com/streamlabs/search/' + document.getElementById('search').value + '')
        .then(response => response.json())
        .then(data => {
            if (data.cloudbot.data.length == 0) {
                ...
                if (data.cloudbot.data.length == 1) {
                    document.getElementById('name1').title = data.cloudbot.data[0].viewer.name
                    document.getElementById('name1').innerHTML = data.cloudbot.data[0].viewer.name
                    fetch('https://backend.mgcounts.com/youtube/user/' + data.cloudbot.data[0].viewer.platform_id + '')
                        .then(response => response.json())
                        .then(data => {
                            document.getElementById('img1').src = data.image
                        })
                } else if (data.cloudbot.data.length == 2) {
                    document.getElementById('name1').title = data.cloudbot.data[0].viewer.name
                    document.getElementById('name1').innerHTML = data.cloudbot.data[0].viewer.name
                    document.getElementById('name2').title = data.cloudbot.data[1].viewer.name
                    document.getElementById('name2').innerHTML = data.cloudbot.data[1].viewer.name
                    fetch('https://backend.mgcounts.com/youtube/user/' + data.cloudbot.data[0].viewer.platform_id + '')
                        .then(response => response.json())
                        .then(data => {
                            document.getElementById('img1').src = data.image
                        })
                        fetch('https://backend.mgcounts.com/youtube/user/' + data.cloudbot.data[1].viewer.platform_id + '')
                        .then(response => response.json())
                        .then(data => {
                            document.getElementById('img2').src = data.image
                        })
                } else if (data.cloudbot.data.length == 3) {
                    document.getElementById('name1').title = data.cloudbot.data[0].viewer.name
                    document.getElementById('name1').innerHTML = data.cloudbot.data[0].viewer.name
                    document.getElementById('name2').title = data.cloudbot.data[1].viewer.name
                    document.getElementById('name2').innerHTML = data.cloudbot.data[1].viewer.name
                    document.getElementById('name3').title = data.cloudbot.data[2].viewer.name
                    document.getElementById('name3').innerHTML = data.cloudbot.data[2].viewer.name
                    fetch('https://backend.mgcounts.com/youtube/user/' + data.cloudbot.data[0].viewer.platform_id + '')
                        .then(response => response.json())
                        .then(data => {
                            document.getElementById('img1').src = data.image
                        ...
                        fetch('https://backend.mgcounts.com/youtube/user/' + data.cloudbot.data[2].viewer.platform_id + '')
                        .then(response => response.json())
                        .then(data => {
